...
export default function AnalyticsPage() {
  ...
  // Category distribution for DonutChart
  const categoryData = useMemo(() => {
    ...
    return Object.entries(counts).map(([name, value], idx) => ({
      name,
      value,
      percentage: Math.round((value / (totalTasks || 1)) * 100),
  ...
  return (
    <div className="space-y-5 pb-8 w-full min-w-0">
      {/* Student Top Header */}
      <div className="overflow-hidden rounded-2xl border border-border bg-card p-5 sm:p-6 shadow-md">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 min-w-0">
          ...
          <span className="self-start sm:self-auto shrink-0 rounded-full bg-primary/10 border border-primary/20 px-3.5 py-1.5 text-xs font-bold text-primary flex items-center gap-2 shadow-2xs">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 min-w-0">
        <div className="rounded-2xl border border-border bg-card p-4.5 transition-all hover:border-primary/40 min-w-0 overflow-hidden shadow-2xs">
          <div className="flex items-center justify-between gap-2 min-w-0">
            <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground truncate">Available Tasks</p>
          ...
          <h2 className="mt-3 text-3xl font-extrabold text-foreground">{totalTasks}</h2>
          <p className="mt-1 text-xs font-medium text-muted-foreground truncate">Tasks open on campus</p>
        </div>

        <div className="rounded-2xl border border-border bg-card p-4.5 transition-all hover:border-primary/40 min-w-0 overflow-hidden shadow-2xs">
          <div className="flex items-center justify-between gap-2 min-w-0">
            <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground truncate">Applied Tasks</p>
          ...
          <h2 className="mt-3 text-3xl font-extrabold text-foreground">{appliedCount}</h2>
          <p className="mt-1 text-xs font-medium text-muted-foreground truncate">Applications submitted</p>
        </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-4.5 transition-all hover:border-primary/40 min-w-0 overflow-hidden shadow-2xs">
          ...
          <h2 className="mt-3 text-3xl font-extrabold text-foreground">{completedCount}</h2>
          <p className="mt-1 text-xs font-medium text-muted-foreground truncate">{completionRate}% Completion Success</p>
        </div>

        <div className="rounded-2xl border border-border bg-card p-4.5 transition-all hover:border-primary/40 min-w-0 overflow-hidden shadow-2xs">
          <div className="flex items-center justify-between gap-2 min-w-0">
            <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground truncate">Open Opportunities</p>
          ...
          <h2 className="mt-3 text-3xl font-extrabold text-foreground">{pendingCount}</h2>
          <p className="mt-1 text-xs font-medium text-muted-foreground truncate">Tasks ready to apply</p>
        </div>
      </div>
          ...
          <div className="space-y-3 text-xs leading-relaxed text-muted-foreground min-w-0">
            <div className="rounded-xl bg-secondary/60 p-3 border border-border/50 flex items-start gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-0.5">Applied for <strong className="text-foreground">{appliedCount}</strong> of <strong className="text-foreground">{totalTasks}</strong> tasks ({Math.round((appliedCount / (totalTasks || 1)) * 100)}%).</p>
              </div>
            </div>
            ...
            <div className="rounded-xl bg-secondary/60 p-3 border border-border/50 flex items-start gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-0.5"><strong className="text-foreground">{completionRate}%</strong> of applied tasks completed.</p>
              </div>
            </div>
