...
def generate_token(user_id, role, college_code):
    payload = {
        'uid': user_id,
        'role': role,
...
def require_admin(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if not user or user.get('role') != 'admin':
            return jsonify({'message': 'Admin privileges required for this action'}), 403
        return f(user, *args, **kwargs)
    return decorated
...
def send_2factor_otp(phone_number, otp_code):
    ...
    """
    if not TWOFACTOR_API_KEY or TWOFACTOR_API_KEY.startswith('your_'):
        return {'success': False, 'message': '2Factor API key not configured'}
    ...
    url = f"https://2factor.in/API/V1/{TWOFACTOR_API_KEY}/SMS/+{clean_phone}/{otp_code}/OTP1"
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as response:
            ...
            return {'success': False, 'message': resp_json.get('Details', 'SMS Error')}
    except Exception as e:
        print("2Factor API Exception:", e)
...
def verify_student_uid():
    ...
    try:
        ...
        send_2factor_otp(student.get('phone'), otp_code)
...
def get_student_details(student_id):
    try:
        ...
        print("DB Error get_student_details:", e)
        return jsonify({'message': 'Student not found'}), 404
...
def update_student_details(student_id):
    ...
    if auth_user and auth_user.get('role') == 'student' and auth_user.get('uid', '').upper() != student_id.upper():
        return jsonify({'message': 'Unauthorized profile update request'}), 403
    ...
    try:
        ...
        if portfolio_link is not None:
            ...
            if port:
                ...
                cur.execute(
                    'INSERT INTO Portfolio (studentid, completedtasks, totalcredits, portfoliolink) VALUES (%s, 0, 0, %s)',
        ...
        print("DB Error update_student_details:", e)
...
def get_all_students():
    ...
    try:
        ...
        if department and department.lower() != 'all':
            cur.execute(
                'SELECT studentid, name, email, department, semester, creditcoins, collegecode FROM Student WHERE UPPER(department) = UPPER(%s) ORDER BY name ASC',
                (department,)
            )
        else:
        ...
        fallback = [
            {'id': '2023CSE045', 'studentID': '2023CSE045', 'name': 'Aarav Patel', 'email': 'aarav@kjsce.edu', 'department': 'Computer Science', 'semester': 5, 'ccBalance': 150},
            {'id': '2023CSE012', 'studentID': '2023CSE012', 'name': 'Ananya Sharma', 'email': 'ananya@kjsce.edu', 'department': 'Computer Science', 'semester': 5, 'ccBalance': 210},
            {'id': '2023IT008', 'studentID': '2023IT008', 'name': 'Rohan Mehta', 'email': 'rohan@kjsce.edu', 'department': 'IT Dept', 'semester': 3, 'ccBalance': 180},
            {'id': '2023BSC004', 'studentID': '2023BSC004', 'name': 'Priya Singh', 'email': 'priya@kjsce.edu', 'department': 'BSCIT', 'semester': 4, 'ccBalance': 120},
        ]
...
def get_leaderboard():
    try:
        ...
        print("DB Error get_leaderboard:", e)
        return jsonify([]), 200


# ---------------------------------------------------------
# Credit Coin Allocation & Admin Operations
# ---------------------------------------------------------
...
def distribute_cc():
    ...
    if not task_id or cc is None or not isinstance(student_ids, list) or len(student_ids) == 0:
        return jsonify({'message': 'taskId, valid cc amount, and non-empty studentIds array are required'}), 400

    if cc < 0:
        return jsonify({'message': 'Credit coin allocation amount cannot be negative'}), 400

    task_title = 'Campus Event / Task'
    ...
    try:
        ...
        # Update task CC
        cur.execute('UPDATE Task SET creditcoins = %s WHERE taskid = %s RETURNING *', (cc, task_id))
        ...
        print("DB Transaction Error in distribute_cc:", e)
        return jsonify({'message': 'Failed to distribute CC due to a transaction error'}), 500
    ...
    try:
        ...
        for t in tasks:
            result.append({
                ...
                'department': t.get('department') or 'Computer Science',
                'urgent': t.get('urgent') or False,
                'category': t.get('category') or 'General',
                'tags': ['Campus', 'Activity']
            })
        return jsonify(result), 200
    except Exception as e:
...
def create_task():
    ...
    if not title or not description:
        return jsonify({'message': 'Task title and description are required'}), 400
...
def get_student_applications(student_id):
    try:
        ...
        print("DB Error get_student_applications:", e)
        return jsonify([]), 200
...
def apply_for_task():
    ...
    try:
        ...
        print("DB Error apply_for_task:", e)
...
def unapply_for_task():
    ...
    try:
        ...
        if application['status'] == 'Completed':
            cur.close()
            conn.close()
            return jsonify({'message': 'Cannot withdraw application for a completed task'}), 400
        ...
        print("DB Error unapply_for_task:", e)
...
def get_admin_applications(admin_id):
    try:
        ...
        print("DB Error get_admin_applications:", e)
        return jsonify([]), 200
...
def handle_admin_application_action():
    ...
    try:
        ...
        print("DB Transaction Error handle_admin_application_action:", e)
        return jsonify({'message': 'Failed to process application status change'}), 500
...
def complete_application():
    ...
    try:
        ...
        task = cur.fetchone()
        task_cc = task['creditcoins'] if task else 50
        ...
        print("DB Transaction Error complete_application:", e)
        return jsonify({'message': 'Failed to complete task'}), 500
...
def get_portfolio(student_id):
    try:
        ...
        cur.execute('''
            SELECT a.applicationid, a.taskid, a.applieddate, t.title, t.creditcoins
            FROM Application a
            JOIN Task t ON a.taskid = t.taskid
        ...
        for row in apps:
            items.append({
                ...
                'taskId': row['taskid'],
                'title': row['title'],
                'ccEarned': row['creditcoins'],
                'completedDate': format_date(row['applieddate'])
            })
        ...
        print("DB Error get_portfolio:", e)
        return jsonify({
            'studentId': student_id,
            'completedTasks': 0,
            'totalCredits': 0,
            'portfolioLink': f"https://portfolio.example.com/{student_id}",
            'items': []
        }), 200


# ---------------------------------------------------------
# Chat & Messaging System (50 Words Limit Enforcement)
# ---------------------------------------------------------
...
def get_chat_contacts():
    ...
    try:
        ...
        print("DB Error get_chat_contacts:", e)
        if role == 'student':
            contacts = [{
                'id': 'ADM001',
                'name': 'Wilson Rao',
...
def get_chat_messages():
    ...
    try:
        ...
        print("DB Error get_chat_messages:", e)
        filtered = [
...
def send_chat_message():
    ...
    if word_count > 50:
        return jsonify({
            'message': f'Message exceeds word limit! Max 50 words allowed per message. (Current: {word_count} words)'
        }), 400
    ...
    try:
        ...
        print("DB Error send_chat_message:", e)
